...
class ServiceCategoryTransformer extends TransformerAbstract
{
    ...
    public function getActionButtons($service_category)
    {
        ...
        data-action='".route('admin.job-title.custom_update' , $service_category->id)."' data-method='POST' data-service-category='".json_encode($service_category)."' data-is-create='false'><i class='fa fa-edit'></i></button>
        <button type='button' data-toggle='modal' data-target='#delete-modal' class='btn-xs btn-danger'
        data-delete-url='".route('admin.job-title.destroy' , $service_category->id)."' data-message='".__('custom.confirm_delete')."' data-name='".$service_category->name."' id='row-".$service_category->id."'><i class='fa fa-trash'></i></button>
        ";
    }
}
